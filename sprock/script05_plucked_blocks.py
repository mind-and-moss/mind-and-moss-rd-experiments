# ============================================================================
# MONSTER8 — SCRIPT 05: PLUCKED AND RELIEVED BLOCK ASSEMBLY
#
...
for bi, (bedname, frac, hard) in enumerate(STACK_S7, start=1):
    ...
    d0, k = 0.0, 0
    while d0 < TOTAL - 1.0:
        # FIX 3 - size variation. Was +/-25%, which read as brickwork.
        # Real block sizes vary hard, and the big ones anchor the composition.
        L = spacing * (0.45 + 1.35 * (h(bi, k, 1) ** 1.7))
        L = min(L, TOTAL - d0)
        if L < 6.0: break
        mid = d0 + L/2
        p, dirv, n, s = at(mid)
        ...
        pluck_p = {5: 0.05, 4: 0.09, 3: 0.14, 2: 0.22, 1: 0.34}[hard] + 0.20*prox_p
        ...
        if in_opening(s, bi) is None and L > 8.0 and not plucked:
            ...
            if in_opening(s, bi) is None and L > 8.0 and plucked: plucked_n += 1
        d0 += L; k += 1
    z += thick

print(f"\n--- MONSTER8 block assembly ---")
print(f"blocks built : {made}   (plucked: {plucked_n})")
for bi,(bedname,frac,hard) in enumerate(STACK_S7, start=1):
